...
<body class="max-w-[1920px] max-h-[1200px] mx-auto">
  ...
  function buildSourceBootstrapScript(browserBootstrapData) {
      ...
      results.forEach((result) => {
        if (result.type === 'category') {
          if (!categoryResults[result.key]) {
            categoryResults[result.key] = { name: result.name, subcategories: [], isSearchMatch: true };
          }
          return;
        }
        if (!categoryResults[result.categoryKey]) {
          categoryResults[result.categoryKey] = { name: settingsTree[result.categoryKey].name, subcategories: [], isSearchMatch: false };
        }
    ...
    function initDeviceWebSocket() {
      ...
      deviceWebSocket.addEventListener('message', (event) => {
        try {
          ...
          const explicitMachines = Array.isArray(data.foundMachines) ? data.foundMachines : null;
          const explicitScales = Array.isArray(data.foundScales) ? data.foundScales : null;
          deviceStateCache.devices = Array.isArray(data.devices) ? data.devices : [];
          deviceStateCache.foundMachines = explicitMachines;
          deviceStateCache.foundScales = explicitScales;
    ...
    function getBluetoothDevicesByType() {
      ...
      if (explicitMachines || explicitScales) {
        return { machines: explicitMachines || [], scales: explicitScales || [] };
      }
      const devices = Array.isArray(deviceStateCache.devices) ? deviceStateCache.devices : [];
      const machines = devices.filter((device) => device && device.type === 'machine');
      const scales = devices.filter((device) => device && device.type === 'scale');
    ...
    window.handleReaSettingSave = function(key, value) {
    ...
    window.handleMachineSettingSave = function(key, value) {
    ...
    window.handleCalibrationSettingSave = function(key, value) {
    ...
    window.handleAdvancedSettingSave = function(key, value) {
...
<div id="source-templates" style="display:none"><div id="presence-template" style="display:none">${renderPresenceSection({ presenceSettings, activeHours })}</div>
